...
namespace EthereumXplorer.Config
{
	public static class ConfigurationExtensions
	{
		...
		{
			string str = configuration[key] ?? configuration[key.Replace(".", string.Empty, StringComparison.InvariantCulture)];
			if (str == null)
			{
				return defaultValue;
			}
			...
			{
				int separator = str.LastIndexOf(":", StringComparison.InvariantCulture);
				if (separator == -1)
				{
					throw new FormatException();
				}

				string ip = str.Substring(0, separator);
				string port = str.Substring(separator + 1);
